...
:loop
set ANY=
for /f "usebackq delims=" %%F in (`powershell -NoProfile -Command "Add-Type -AssemblyName System.Windows.Forms; $d = New-Object System.Windows.Forms.OpenFileDialog; $d.Title = 'Selectionner une ou plusieurs images a iconifier'; $d.Filter = 'Images PNG (*.png)|*.png|Toutes les images|*.png;*.jpg;*.jpeg;*.webp'; $d.Multiselect = $true; $d.InitialDirectory = '%INITIAL_DIR%'; if ($d.ShowDialog() -eq 'OK') { $d.FileNames }"`) do (
    echo Fichier : %%F
    "%PYTHON%" "%~dp0iconify.py" "%%F"
    set ANY=1
    set "INITIAL_DIR=%%~dpF"
)
